...
class UDPCompare extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
    ...
    Object.keys(organized_data).forEach((dataset_label, i) => {
      ...
      Object.keys(data_set).forEach(key => {
        ...
        const max_value = data_set[key].reduce((out, current) => {
          let new_out = out
          Object.keys(current.frame_size_results).forEach(size => {
            if (current.frame_size_results[size].actual_mbps > new_out) {
              new_out = current.frame_size_results[size].actual_mbps
            }
          })
          return new_out
        }, 0)
      ...
      <div>
        ...
        <div className='chart-section'>
          ...
          <BarChart 
            ref='pf_chart'
            height={100}
            data={pass_fail_chart_data} 
            options={pf_bar_graph_options}  
            redraw 
          />
        </div>
