...
class MaxClientCompare extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  preRender() {
    ...
    // MAX clients achieved
    const max_clients_chart_data = {
      labels: new Set(),
      datasets: []
    }
    ...
    Object.keys(organized_data).forEach((dataset_label, i) => {
      ...
      Object.keys(data_set).forEach(key => {
        max_clients_chart_data.labels.add(key)

        const max_value = data_set[key].reduce((out, current) => {
          let new_out = out
          if (current.clients > out) {
            new_out = current.clients
          }
      ...
      Object.assign(bundle, bar_chart_colors[i % bar_chart_colors.length])
      max_clients_chart_data.datasets.push(bundle)
    })
    max_clients_chart_data.labels = Array.from(max_clients_chart_data.labels)
    ...
    return { pass_fail_chart_data, max_clients_chart_data, total_tests_chart_data }
  }

  render() {
    const { pass_fail_chart_data, max_clients_chart_data, total_tests_chart_data } = this.preRender()
    return (
      <div>
        ...
        <div className='chart-section'>
          <h2 className='compare-res-title'>Max Number of Clients</h2>
          <PrintChartButton
            getUri={getUri('max_chart').bind(this)}
            file_name={`${this.props.test_type}_max_clients_by_${this.state.organize_by}`} 
          />
          <BarChart 
            ref={'max_chart'}
            height={100}
            data={max_clients_chart_data} 
            options={{
              scales: {
                yAxes: [{      
                  display: true,
        ...
        <div className='chart-section'>
          ...
          <BarChart 
            ref='total_chart'
            height={100}
            data={total_tests_chart_data} 
            options={total_tests_options} 
            redraw 
          />
        </div>
...
export default MaxClientCompare
